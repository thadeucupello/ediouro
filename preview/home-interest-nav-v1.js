...
home = function(){
 ...
 const heroSecond = W['os-assassinatos-no-honjin'] || W['eu-te-amei-em-outra-vida'] || W['o-caminho-dos-reis'];
 out = out.replace('<div class="hero-cover hero-placeholder"><div>Em breve<span>novo destaque</span></div></div>',heroSecond?`<div class="hero-cover real" onclick="go('/livros/${heroSecond.slug}')">${cover(heroSecond)}</div>`:'');
 out = out.replaceAll('<div class="placement-badge">Placement editorial · imagem/campanha</div>','');
 out = out.replace('Mistborn, O Arquivo das Tempestades e outros mundos de Brandon Sanderson fazem parte de uma história maior. Este espaço passa a funcionar como um placement editorial administrável: imagem, vídeo, campanha, lançamento ou guia.','Mistborn, O Arquivo das Tempestades e outros mundos de Brandon Sanderson fazem parte de uma história maior. Veja por onde começar, como as séries se conectam e encontre sua próxima leitura no Cosmere.');
 out = out.replace('Um segundo placement mostra como a home pode ganhar campanhas temporárias sem virar uma coleção de banners. No WordPress, este bloco poderá trocar imagem, título, CTA e período de exibição.','Para quem gosta de pistas, suspeitos e reviravoltas, reunimos três títulos do catálogo para continuar a investigação.');
 out = out.replace('Uma casa editorial de catálogo amplo dentro de uma experiência única de descoberta.','Clássicos, literatura brasileira e estrangeira, não ficção e obras que continuam encontrando novos leitores.');
 return out;
};
(function(){
 const css=`.interest-nav-head>div:first-child{max-width:620px}.interest-nav-head p{margin:10px 0 0;color:#6e665e;font:17px/1.45 'Newsreader',Georgia,serif}.interest-card{cursor:pointer;position:relative;display:flex!important;flex-direction:column;justify-content:space-between;gap:24px;transition:transform .18s ease,box-shadow .18s ease}.interest-card:hover,.interest-card:focus-visible{transform:translateY(-4px);box-shadow:0 14px 28px rgba(35,31,26,.14);outline:none}.interest-card-cta{font:700 9px/1.2 'Archivo',Arial,sans-serif;letter-spacing:.12em;text-transform:uppercase;opacity:.78}.interest-card:hover .interest-card-cta,.interest-card:focus-visible .interest-card-cta{opacity:1}.placement-media:has(.placement-collage) .placement-collage{height:100%}@media(max-width:760px){.interest-nav-head{align-items:flex-start}.interest-cats{display:flex!important;overflow-x:auto;scroll-snap-type:x mandatory;gap:10px;padding-bottom:8px;margin-right:-20px;padding-right:20px}.interest-cats .interest-card{min-width:220px;scroll-snap-align:start}.interest-card:hover{transform:none}}`;
